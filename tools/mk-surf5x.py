...
!!!!! good
Оболочка
версия ленточной протяжки и интерполяции. слава чатжпт + ручного тюнинга)

ВАЖНО
1. Учитывается что первый файл содержит только 1 линию
2. Последний файл, который линия, должен иметь столько же точек сколько и предпоследний, пусть P,
и плюс там должно быть P/2 совпадающих точек с одной стороны и P/2 точек с другой.
...
def loadN( N ):
    ...
    for i in range(0,N):
        ...
        with open(fname, "r") as file:
            ...
            if i == 0: # первая полоска вырожденный случай - добавим вторую половинку идущую обратно
                kk = len(coordinates)
                for j in range(kk):
